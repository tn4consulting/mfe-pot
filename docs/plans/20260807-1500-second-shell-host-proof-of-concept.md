...
- Distinct identity end to end: Nx project `job-bank-shell`, federation name `job-bank-shell`, PKCE `CLIENT_ID = 'mfe-pot-job-bank-shell'`, dev-server port 4205 (so both hosts run side by side locally without colliding with `msca-shell`'s 4200), Helm release `job-bank-shell`. Ingress host went through two iterations: initially `job-bank-shell.mfe-pot.local` (matching the repo name), then `job-bank.mfe-pot.local` (the plain "front door" name — with every internal remote's own Ingress host correspondingly suffixed `-mfe`, e.g. `job-bank-mfe.mfe-pot.local`, to keep the front door and the remote it composes visually distinct).
