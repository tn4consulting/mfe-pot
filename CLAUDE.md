...
| `mfe-pot-job-bank-shell` | Second, minimal proof-of-concept host app — composes only the job-bank remote's own `./Component`, no sidebar nav (one destination). No BFF. Proves the federation pattern generalizes to more than one host. |
